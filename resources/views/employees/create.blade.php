                    ...
                    <form method="post" action="{{ route('employees.store') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input id="last_name" class="block mt-1 w-full" type="text" name="last_name" value="{{ old('last_name') }}" required autofocus />
                                @error('last_name')
                                <div class="text-red-500 mt-2 text-sm">
                                    {{ $message }}
                                </div>
                            ...
                            <div>
                                ...
                                <input id="phone" class="block mt-1 w-full" type="tel" name="phone" value="{{ old('phone') }}" required autofocus />
                                @error('phone')
                                <div class="text-red-500 mt-2 text-sm">
                                    {{ $message }}
                                </div>
                            ...
                            <div>
                                ...
                                <input id="salary" class="block mt-1 w-full" type="text" name="salary" value="{{ old('salary') }}" required autofocus />
                                @error('salary')
                                <div class="text-red-500 mt-2 text-sm">
                                    {{ $message }}
                                </div>
                            ...
                            <div>
                                ...
                                <select id="managers" name="manager_id" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm">
                                    @foreach($departments as $department)
                                        ...
                                        <option value="{{ $department->manager->id }}" {{ old('manager_id') == $department->manager->id ? 'selected' : '' }}>{{ $department->manager->full_name }}</option>
                                        @endif
                                    @endforeach
                                </select>
                                @error('manager_id')
